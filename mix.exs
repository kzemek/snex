defmodule Snex.MixProject do
    ...
    [
      ...
      docs: [
        ...
        before_closing_head_tag: fn
          :html -> ~s'<script src="script/rename_python_interface_docs.js"></script>'
          _ -> ""
        end
      ]
    ]
  ...
  defp gen_python_doc(_) do
    ...
    dest_path = Path.join(["lib", "python_interface.ex"])
  ...
  defp clean_python_doc(_) do
    Path.join(["lib", "python_interface.ex"])
    |> File.rm_rf!()

    Path.join([Mix.Project.build_path(), "ebin", "Elixir.Python_Interface_Docs.beam"])
    |> File.rm_rf!()
  end
end
